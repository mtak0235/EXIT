...
model=torch.load("C:/Users/jesus/OneDrive/바탕 화면/bmw_4.pt",map_location = torch.device ( 'cpu'))

#데이터 셋 읽고 답지 만들기


f=open("result1.txt","r",encoding="utf-8")
data=f.read()
...
test_df["cafe"]=0.5
test_df["cat"]=0.5
test_df.to_csv("./result_1.csv",mode="w",encoding="utf-8")

#데이셋 분류

BATCH_SIZE=32
test_df = pd.read_csv('./result.csv')
submission = pd.read_csv('./result_1.csv')
